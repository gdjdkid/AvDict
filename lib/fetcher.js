...
async function searchJavdb(id) {
    try {
        ...
        // 用 spawnSync 传数组参数，完全不经过 shell
        // 特殊字符 +/= 直接传给 curl，不会被 shell 破坏
        function fetchJavdb(url) {
        ...
        const searchHtml = fetchJavdb(searchUrl);
        ...
        for (let i = 0; i < 3; i++) {
            ...
            detailHtml = fetchJavdb(`https://javdb.com${detailPath}`);
