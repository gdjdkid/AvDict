...
function searchJavdb(id) {
    try {
        ...
        // 加这几行调试
        console.error('搜索HTML长度:', searchHtml.length);
        const allItems = searchHtml.match(/class="box"/g);
        console.error('找到box元素数量:', allItems?.length || 0);
        const allStrong = searchHtml.match(/<strong>[^<]+<\/strong>/g);
        console.error('找到strong元素:', allStrong?.slice(0,5));
        ...
        console.error('JAVDB查询错误:', e.message);
        return null;
    }
}
...
export async function search(id) {
    // 自动识别 FC2 格式：031926_100 → FC2-031926
    if (/^\d{5,6}_\d+$/.test(id)) {
        id = `FC2-${id.split('_')[0]}`;
